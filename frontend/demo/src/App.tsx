...
import arrowDiagonalIcon from '../../icons/arrow-diagonal.svg?raw'
import canvasIcon from '../../icons/canvas.svg?raw'
...
import documentListIcon from '../../icons/document-list.svg?raw'
import downloadIcon from '../../icons/download.svg?raw'
import entityCollectionIcon from '../../icons/entity-collection.svg?raw'
import expandIcon from '../../icons/expand.svg?raw'
import filterIcon from '../../icons/filter.svg?raw'
import flameIcon from '../../icons/flame.svg?raw'
import generateIcon from '../../icons/generate.svg?raw'
...
import membersIcon from '../../icons/members.svg?raw'
import neuralNetworkIcon from '../../icons/neural-network.svg?raw'
import pauseIcon from '../../icons/pause.svg?raw'
import playBoxedIcon from '../../icons/play-boxed.svg?raw'
import playIcon from '../../icons/play.svg?raw'
import playNextIcon from '../../icons/play-next.svg?raw'
import plusIcon from '../../icons/plus.svg?raw'
import profileIcon from '../../icons/profile.svg?raw'
import searchIcon from '../../icons/search.svg?raw'
import searchV2Icon from '../../icons/search-v2.svg?raw'
import shareIcon from '../../icons/share.svg?raw'
import speechIcon from '../../icons/speech.svg?raw'
import spinnerIcon from '../../icons/spinner.svg?raw'
import transcriptionIcon from '../../icons/transcription.svg?raw'
import trophyIcon from '../../icons/trophy.svg?raw'
import usageIcon from '../../icons/usage.svg?raw'
import visionDisabledIcon from '../../icons/vision-disabled.svg?raw'
import visionIcon from '../../icons/vision.svg?raw'
import volumeMidIcon from '../../icons/volume-mid.svg?raw'
import warningIcon from '../../icons/warning.svg?raw'

const icons: Record<string, string> = {
  analyze: analyzeIcon,
  'arrow-box-right': arrowBoxRightIcon,
  ...
  'entity-collection': entityCollectionIcon,
  expand: expandIcon,
  filter: filterIcon,
  flame: flameIcon,
  ...
  pause: pauseIcon,
  play: playIcon,
  'play-boxed': playBoxedIcon,
  'play-next': playNextIcon,
  plus: plusIcon,
  profile: profileIcon,
  search: searchIcon,
  'search-v2': searchV2Icon,
  share: shareIcon,
  speech: speechIcon,
  spinner: spinnerIcon,
  ...
  trophy: trophyIcon,
  usage: usageIcon,
  'vision-disabled': visionDisabledIcon,
  vision: visionIcon,
...
const WORKSPACE_METADATA_TIMEOUT_MS = 20000
...
function App() {
  ...
  const workspaceMetadataRequestRef = useRef<Record<string, Promise<JockeyWorkspaceMetadataResponse> | undefined>>({})
  ...
  const requestWorkspaceMetadata = useCallback((tag: string, videoName: string, force = false) => {
    ...
    const request = fetchJsonWithTimeout<JockeyWorkspaceMetadataResponse>(
      `/games/${encodeURIComponent(tag)}/videos/${encodeURIComponent(videoName)}/jockey-workspace${query ? `?${query}` : ''}`,
      undefined,
      WORKSPACE_METADATA_TIMEOUT_MS,
  ...
  useEffect(() => {
    if (view !== 'workspace' || !selectedTag || !activeVideoName) {
      setWorkspaceMetadataLoadingKey('')
      setWorkspaceMetadataError('')
      return
    }

    if (!activeIndexVideo && isLoadingIndexVideos) return
    const metadataCacheKey = `${selectedTag}|${activeVideoName}`
    const hasLocalWorkspaceMetadata = Boolean(workspaceMetadataByKeyRef.current[metadataCacheKey])
    if (activeIndexVideo?.has_jockey_workspace_metadata === false && !hasLocalWorkspaceMetadata) {
      setWorkspaceMetadataLoadingKey('')
      setWorkspaceMetadataError('')
      return
    }
  ...
  }, [activeIndexVideo, activeVideoName, isLoadingIndexVideos, requestWorkspaceMetadata, selectedTag, view, workspaceMetadataRefreshToken])
          ...
          <div className="app-header-inner mx-auto w-full max-w-[1920px]">
            ...
            <nav className="app-header-nav" aria-label="Main navigation">
              ...
              <div className="app-header-actions">
                ...
                >
                  <AppIcon name="help" className="h-4 w-4 shrink-0" />
        ...
        <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {step.actionLabel && (
              ...
              >
                <AppIcon name={step.id === 'jockey-chat' ? 'speech' : 'generate'} className="h-3.5 w-3.5" />
                {step.actionLabel}
              </button>
            )}
            <button
            ...
            >
              {nextLabel}
              <AppIcon name={stepIndex >= totalSteps - 1 ? 'checkmark' : 'arrow-box-right'} className="h-3.5 w-3.5" />
            </button>
          </div>
        </div>
      </section>
    ...
    >
      <div className="relative z-10 mx-auto flex w-full max-w-[1920px] flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
        <div className="flex min-w-0 items-center gap-2">
          ...
          >
            <AppIcon name={activeCategory.icon} className="h-4 w-4" />
          </span>
          <div className="min-w-0">
          ...
          {categories.map((category) => {
            ...
            return (
              ...
              >
                ...
                >
                  <AppIcon name={category.icon} className="h-3 w-3" />
                </span>
        ...
        <div className="flex aspect-video items-center justify-center text-text-primary">
          {hasPlayable && streamInfoUrl ? (
            ...
            <div className="flex h-full w-full flex-col items-center justify-center gap-3 bg-card px-6 text-center">
              <AppIcon name="info" className="h-7 w-7 text-text-tertiary" />
        ...
        <div className="grid gap-4 bg-card px-5 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <AppIcon name="play-boxed" className="h-4 w-4 text-accent" />
                ...
                {assemblyVideoReady && (
                  <div className="pointer-events-none absolute left-3 top-3 z-[4] flex max-w-[calc(100%-24px)] items-center gap-2">
                    <span className="inline-flex h-8 max-w-full items-center gap-2 rounded-md border border-accent/70 bg-accent px-2.5 text-xs font-semibold text-brand-charcoal shadow-[0_8px_18px_rgba(0,0,0,0.18)]">
                      <AppIcon name="checkmark" className="h-3.5 w-3.5 shrink-0" />
          ...
          {sequenceClips.map((clip, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  <AppIcon name={laneIcon} className="h-4 w-4" />
                </span>
              </button>
            )
          })}
    ...
    <section className="jockey-chat flex min-h-0 w-full flex-1 flex-col">
              ...
              {jockeyProducerSkills.map((skill) => (
                ...
                >
                  <span className="jockey-chat-suggestion-icon inline-flex items-center justify-center" style={{ color: skill.color }}>
                    <AppIcon name={skill.icon} className="h-5 w-5" />
                  </span>
      ...
      <div className="jockey-chat-composer-shell">
        <div data-tour-id="jockey-composer" className="jockey-chat-composer">
          ...
          >
            <AppIcon name={loading ? 'spinner' : 'generate'} className={['h-4 w-4', loading ? 'animate-spin' : ''].join(' ')} />
  ...
  return (
    <div className="jockey-chat-response jockey-chat-response-block mt-4 rounded-md border border-border-light bg-surface shadow-[0_1px_2px_rgba(31,41,33,0.035)]">
      <div className="jockey-chat-response-header">
        <div className="flex min-w-0 items-center gap-2">
          <AppIcon name={skill?.icon || 'speech'} className="h-4 w-4 shrink-0 text-accent" />
...
function JockeyManifestSummary({
  ...
  return (
    <div className="jockey-chat-response jockey-chat-response-block rounded-md border border-border-light bg-surface shadow-[0_1px_2px_rgba(31,41,33,0.035)]">
      <div className="jockey-chat-response-header">
        <div className="flex min-w-0 items-center gap-2">
          <AppIcon name={skill?.icon || 'document-list'} className="h-4 w-4 shrink-0 text-accent" />
...
function JockeySaveTurnButton({
  ...
  return (
    <div className="jockey-chat-save flex shrink-0 flex-col items-stretch gap-1 sm:items-end">
      ...
      >
        <AppIcon name={saving ? 'spinner' : saved ? 'checkmark' : 'document'} className={['h-3.5 w-3.5', saving ? 'animate-spin' : ''].join(' ')} />
            ...
            <div className="jockey-chat-clip-meta px-3 pb-3 pt-1">
              <div className="jockey-chat-clip-meta-row flex min-w-0 flex-wrap items-center gap-2">
                ...
                {workspaceMoment && sourceName && (
                  ...
                  >
                    <AppIcon name="arrow-diagonal" className="h-3 w-3" />
                  </button>
                )}
              </div>
      ...
      <section className="dashboard-explainability dashboard-explainability--collapsed pointer-events-auto">
        ...
        >
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-sm border border-accent bg-accent-light text-brand-charcoal">
            <AppIcon name="checkmark" className="h-4 w-4" />
          </span>
      ...
      >
        {poster ? (
          ...
          <div className="flex h-full w-full items-center justify-center bg-surface">
            <AppIcon name="play-boxed" className="h-9 w-9 text-accent" />
          </div>
        )}
  ...
  return (
    <main className="app-startup-screen min-h-screen bg-background text-text-primary">
      <div className="app-startup-screen-inner mx-auto flex min-h-screen w-full max-w-md flex-col items-center justify-center px-6 py-12 text-center">
        ...
        {status === 'loading' ? (
          <>
            <AppIcon name="spinner" className="mt-8 h-7 w-7 animate-spin text-accent" />
  ...
  return (
    <div className="overview-nav-buttons">
      {variant === 'hero' && onNavigate ? (
        ...
        >
          <AppIcon name="search-v2" className="h-4 w-4" />
          Discover
        </button>
      ) : null}
          ...
          <ul className="overview-narrative-benefits mt-6">
            {overviewSolutionBenefits.map((item) => (
              <li key={item} className="overview-narrative-benefit">
                <AppIcon name="checkmark" className="h-4 w-4 shrink-0 text-mb-green-dark" />
        ...
        <section className="discover-page-results min-w-0">
          <div className="discover-page-results-bar">
            <div className="discover-page-results-label inline-flex items-center gap-3 font-semibold uppercase text-text-tertiary">
              <AppIcon name={searchLoading ? 'spinner' : 'search'} className={['h-4 w-4 shrink-0', searchLoading ? 'animate-spin' : ''].join(' ')} />
          ...
          >
            ...
            <span className="absolute inset-0 flex items-center justify-center opacity-0 transition-opacity group-hover:opacity-100">
              <span className="flex h-12 w-12 items-center justify-center rounded-full border border-accent bg-accent-light text-brand-charcoal shadow-[0_8px_20px_rgba(29,28,27,0.12)]">
                <AppIcon name={canPreviewSegment ? 'play' : 'arrow-diagonal'} className="h-4 w-4" />
              </span>
            </span>
...
function DiscoverBadge({ icon, children }: { icon: string; children: string }) {
  return (
    <span className="inline-flex h-7 max-w-full items-center gap-1.5 rounded-md border border-border-light bg-surface px-2 text-xs font-semibold text-text-secondary">
      <AppIcon name={icon} className="h-3.5 w-3.5 text-accent" />
          ...
          {statusOverlayStyle === 'loader' ? (
            <div className="min-w-[170px] rounded-md border border-white/20 bg-brand-charcoal/82 px-3 py-2 text-white shadow-[0_8px_20px_rgba(0,0,0,0.24)]">
              <div className="flex items-center justify-center gap-2">
                <AppIcon name={status === 'error' ? 'warning' : 'spinner'} className={['h-4 w-4', status !== 'error' ? 'animate-spin' : ''].join(' ')} />
    ...
    return (
      <div className="absolute inset-x-3 bottom-3 z-20">
        <div className="grid grid-cols-[32px_minmax(0,1fr)_52px] items-center gap-2 rounded-md bg-brand-charcoal/82 px-2 py-1.5 text-white shadow-[0_8px_20px_rgba(29,28,27,0.18)] backdrop-blur-sm">
          ...
          >
            <AppIcon name={playing ? 'pause' : 'play'} className="h-3 w-3" />
          </button>
          <button
            type="button"
            onClick={onSeek}
    ...
    <div className="absolute inset-x-3 bottom-3 z-20 rounded-md border border-border bg-surface/96 px-3 py-2 text-text-primary shadow-[0_8px_24px_rgba(29,28,27,0.22)]">
      <div className="grid grid-cols-[34px_minmax(0,1fr)_64px] items-center gap-3">
        <button
          ...
          title={playing ? 'Pause video' : 'Play video'}
        >
          <AppIcon name={playing ? 'pause' : 'play'} className="h-3.5 w-3.5" />
        </button>
        <button
          type="button"
          onClick={onSeek}
        ...
        <div className="grid gap-4 bg-white px-5 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center">
          <div className="min-w-0">
            ...
            <div className="mt-1 flex min-w-0 items-center gap-3">
              <AppIcon name="play-boxed" className="h-5 w-5 shrink-0 text-accent" />
    ...
    return (
      <section className="dashboard-status-card overflow-hidden rounded-md border border-border bg-white shadow-[0_8px_24px_rgba(29,28,27,0.045)]">
        ...
        <div className="border-t border-border-light bg-white px-5 py-4">
          <div className="flex min-w-0 items-center gap-2">
            <AppIcon name="document-list" className="h-4 w-4 shrink-0 text-accent" />
...
function WorkspaceFact({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="inline-grid min-w-0 grid-cols-[auto_minmax(0,1fr)] items-center gap-x-2">
      <AppIcon name={icon} className="row-span-2 h-4 w-4 shrink-0 text-accent" />
      ...
      <div className="flex max-h-[calc(100vh-32px)] w-full max-w-[1180px] flex-col overflow-hidden rounded-md border border-border bg-white shadow-[0_24px_70px_rgba(0,0,0,0.28)]">
        <div className="flex min-w-0 flex-wrap items-center justify-between gap-3 border-b border-border-light bg-white px-4 py-3">
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            >
              <AppIcon name="arrow-diagonal" className="h-4 w-4" />
              Open Dashboard
            </button>
            <button
              type="button"
    ...
    <section className="rounded-md border border-border bg-surface px-4 py-3 shadow-[0_6px_18px_rgba(29,28,27,0.035)]">
      ...
      {loading ? (
        <div className="mt-3 inline-flex items-center gap-2 rounded-sm border border-border-light bg-card px-2.5 py-1.5 text-xs font-semibold text-text-secondary">
          <AppIcon name="spinner" className="h-3.5 w-3.5 animate-spin text-accent" />
      ...
      >
        <div className="flex min-w-0 items-start gap-2">
          <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-sm border border-border-light bg-surface text-accent">
            <AppIcon name={summary.icon} className="h-3.5 w-3.5" />
          </span>
          <div className="min-w-0 flex-1">
            <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          ...
          <AppIcon name={expanded ? 'collapse' : 'expand'} className="mt-1 h-3.5 w-3.5 shrink-0 text-text-tertiary" />
        </div>
      </button>
      ...
      <div className="aspect-video bg-card">
        {streamInfoUrl && range ? (
          ...
          <div className="flex h-full flex-col items-center justify-center gap-2 px-4 text-center">
            <AppIcon name="vision-disabled" className="h-5 w-5 text-text-tertiary" />
        ...
        <ul className="mt-3 grid gap-1.5">
          {evidence.map((value, index) => (
            <li key={`${value}-${index}`} className="grid grid-cols-[auto_minmax(0,1fr)] gap-1.5 text-xs leading-5 text-text-secondary">
              <AppIcon name="checkmark" className="mt-0.5 h-3 w-3 text-accent" />
              <span>{value}</span>
            </li>
          ))}
        </ul>
    ...
    <section id="entity-tracking" data-tour-id="entity-tracking" className="dashboard-entity-section dashboard-entity-workspace scroll-mt-[calc(var(--sj-explainability-top)+24px)] overflow-hidden rounded-md border border-border bg-surface shadow-[0_10px_28px_rgba(29,28,27,0.045)]">
      <div className={['dashboard-entity-header grid gap-4 px-5 py-4 lg:grid-cols-[minmax(0,1fr)_auto] lg:items-center', collapsed ? '' : 'border-b border-border'].join(' ')}>
        ...
        <div className="flex flex-wrap items-center gap-2 lg:justify-end">
          ...
          >
            <AppIcon name={collapsed ? 'expand' : 'collapse'} className="h-3.5 w-3.5" />
            {collapsed ? 'Show' : 'Hide'}
          </button>
        </div>
      </div>
      ...
      <div className="flex min-w-0 items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md border border-border bg-card text-accent">
            <AppIcon name="play-next" className="h-4 w-4" />
          </span>
              ...
              <div className="min-w-0">
                <div className="flex min-w-0 flex-wrap items-center gap-1.5">
                  ...
                  <span className="inline-flex items-center gap-1 rounded-sm border border-accent/35 bg-accent-light px-2 py-1 text-[11px] font-semibold text-brand-charcoal">
                    <AppIcon name="play-next" className="h-3 w-3 text-accent" />
                    {interactionType}
                  </span>
                  <span className="rounded-sm border border-border-light bg-surface px-2 py-1 text-xs font-semibold text-text-secondary">
...
function AnalysisIndexNotice({ game }: { game: Game | null }) {
  return (
    <section className="rounded-md border border-warning bg-warning-light p-5 text-warning-dark shadow-[0_8px_24px_rgba(29,28,27,0.045)]">
      <div className="flex items-start gap-3">
        <AppIcon name="hourglass" className="mt-0.5 h-5 w-5" />
        <div>
            ...
            <div className="flex aspect-video items-center justify-center bg-card text-text-primary">
              {streamInfoUrl ? (
                ...
                <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-6 text-center">
                  <AppIcon name="info" className="h-8 w-8 text-text-tertiary" />
...
function Notice({ tone, icon, text }: { tone: 'neutral' | 'error'; icon: string; text: string }) {
  ...
  return (
    <div className={`flex items-center gap-3 rounded-md border px-4 py-3 text-sm font-medium ${classes}`}>
      <AppIcon name={icon} className={icon === 'spinner' ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} />
      <span>{text}</span>
    </div>
  )
}
...
function AppIcon({ name, className = 'h-4 w-4' }: { name: string; className?: string }) {
  ...
  return <span className={`app-icon shrink-0 ${className}`} dangerouslySetInnerHTML={{ __html: svg }} />
}
...
async function fetchJsonWithTimeout<T>(
  ...
  timeoutMs: number,
  timeoutMessage = 'Backend is still waking up',
): Promise<T> {
  const controller = new AbortController()
