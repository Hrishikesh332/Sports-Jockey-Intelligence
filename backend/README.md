...
deployed environment. The `twelvelabs-ui` app needs `tag`, `label`, `sport`,
...
- UI application code under `../twelvelabs-ui/`
...
When `video_name` is omitted, Jockey may reason across the whole knowledge store. `limit` may be supplied as an explicit clip cap. When `include_reel` is true and `limit` is omitted, numbered prompts such as "find 6 moments" use that requested count; otherwise the app keeps every grounded clip returned by Jockey.
