...
WORKSPACE_METADATA_CACHE_TTL_SECONDS = max(
    0.0,
    float(os.environ.get("SPORTS_WORKSPACE_METADATA_CACHE_TTL_SECONDS", "30")),
)
_WORKSPACE_METADATA_CACHE = {}
_WORKSPACE_METADATA_CACHE_LOCK = Lock()


def get_jockey_workspace_metadata(tag, video_name, lookup=None):
    cache_key = _workspace_metadata_cache_key(tag, video_name, lookup)
    now = time.monotonic()
    if WORKSPACE_METADATA_CACHE_TTL_SECONDS > 0:
        with _WORKSPACE_METADATA_CACHE_LOCK:
            cached = _WORKSPACE_METADATA_CACHE.get(cache_key)
            if cached and cached["expires_at"] > now:
                return deepcopy(cached["payload"])
            if cached:
                _WORKSPACE_METADATA_CACHE.pop(cache_key, None)

    game, target = _resolve_workspace_target(tag, video_name, lookup=lookup)
    ...
    workspace = _load_workspace(
        index_id,
        indexed_asset_id,
        indexed_asset=target.get("indexed_asset"),
    )
    response = {
        "game_tag": game["tag"],
    ...
    if WORKSPACE_METADATA_CACHE_TTL_SECONDS > 0:
        with _WORKSPACE_METADATA_CACHE_LOCK:
            _WORKSPACE_METADATA_CACHE[cache_key] = {
                "expires_at": time.monotonic() + WORKSPACE_METADATA_CACHE_TTL_SECONDS,
...
def _append_item_for_video(tag, video_name, item, lookup=None):
    ...
    _invalidate_workspace_metadata_cache(tag, video_name)
...
def _resolve_workspace_target(tag, video_name, lookup=None):
    from app.services.games import (
        get_game,
        resolve_workspace_video_target,
    )
    ...
    game = get_game(tag)
    lookup_values = lookup if isinstance(lookup, dict) else {}
    target = resolve_workspace_video_target(
        game,
        video_name=video_name,
        lookup=lookup,
        hydrate_metadata=not bool(lookup_values.get("indexed_asset_id")),
        status_code=404,
    )
    indexed_asset_id = target["indexed_asset_id"]
    if not indexed_asset_id:
...
def _load_workspace(index_id, indexed_asset_id, indexed_asset=None):
...
def _workspace_metadata_cache_key(tag, video_name, lookup=None):
    lookup = lookup if isinstance(lookup, dict) else {}
    return (
        str(tag or ""),
        str(video_name or ""),
        str(lookup.get("indexed_asset_id") or ""),
        str(lookup.get("asset_id") or ""),
        str(lookup.get("reference") or ""),
        str(lookup.get("source_video") or ""),
    )


def _invalidate_workspace_metadata_cache(tag, video_name=None):
    if WORKSPACE_METADATA_CACHE_TTL_SECONDS <= 0:
        return
    tag = str(tag or "")
    ...
    with _WORKSPACE_METADATA_CACHE_LOCK:
        stale_keys = [
            key
            for key in _WORKSPACE_METADATA_CACHE
            if key[0] == tag and (not video_name or key[1] == video_name)
        ]
        for key in stale_keys:
            _WORKSPACE_METADATA_CACHE.pop(key, None)
