...
VIDEO_STATS_SCHEMA = {
    ...
    "required": ["summary", "metrics", "events"],
}


def generate_pegasus_video_stats(asset_context, index_id=None, source_video_name=None):
    if not isinstance(asset_context, dict) or not asset_context.get("asset_id"):
        raise ApiError("Pegasus stats generation requires an indexed asset", 400)
    result = twelvelabs_analyze_video(
        pegasus_stats_payload(
            asset_context=asset_context,
            index_id=index_id,
            source_video_name=source_video_name,
        ),
    )
    return parse_pegasus_stats_json(result)


def pegasus_stats_payload(asset_context, index_id=None, source_video_name=None):
    return {
        "model_name": TWELVELABS_PEGASUS_MODEL,
        "video": {
            "type": "asset_id",
        ...
        "response_format": {
            "type": "json_schema",
            "json_schema": pegasus_stats_schema(),
        },
        "max_tokens": 6000,
    }


def pegasus_stats_prompt(asset_context, index_id=None, source_video_name=None):
    source_name = source_video_name or asset_context.get("source_video_name") or asset_context.get("asset_name")
    parts = [
        "Extract a factual sports stat sheet from this video asset using Pegasus 1.5 analysis only.",
        "Do not use a Jockey knowledge-store response or outside assumptions.",
        f"Playable source video name: {source_name}.",
        "Return a single JSON object with these keys: summary, sport, competition, final_result, metrics, events.",
        "summary: one concise sentence describing the match/race and outcome grounded in the footage.",
        "sport and competition: identify only if clearly shown or stated; otherwise return an empty string.",
        "final_result: the final score, finishing order, or result exactly as shown on the scoreboard or graphics; empty string if not shown.",
        "metrics: an array of {label, value} pairs for explicit scoreboard or stat-sheet numbers that are visible or audible (e.g., score, possession, shots, laps, sets, games, fouls, penalties). Use short labels and string values.",
        "events: an array of {time, label, detail} for explicit scoreboard/stat events in chronological order (e.g., goals, scores, penalties, cards, substitutions, lead changes). time is the visible/elapsed clock if shown, else empty string.",
        "Use only explicit, visible or audible facts. Do not invent scores, names, times, standings, or results. If a value is not supported by the footage, omit it rather than guessing.",
        "Keep metrics and events concise; prefer the essential, clearly-supported facts over exhaustive coverage.",
    ]
    if index_id:
        parts.append(f"Configured TwelveLabs index ID: {index_id}.")
    return "\n".join(parts)


def pegasus_stats_schema():
    return strip_unsupported_schema_keywords(VIDEO_STATS_SCHEMA)


def parse_pegasus_stats_json(result):
    nested_result = result.get("result") if isinstance(result, dict) else None
    if isinstance(nested_result, dict):
        try:
            return parse_pegasus_stats_json(nested_result)
        except ApiError:
            pass
    ...
    raise ApiError("TwelveLabs Pegasus analysis did not include valid stats JSON", 502)
