...
from app.core.config import (
    ...
    env_default_game,
    entity_collection_id as configured_entity_collection_id,
...
from app.integrations.twelvelabs import (
    ...
    create_analyze_task as twelvelabs_create_analyze_task,
    create_response as twelvelabs_create_response,
    create_entity_collection_entity as twelvelabs_create_entity_collection_entity,
    delete_indexed_asset,
    delete_knowledge_store_item,
    asset_preview_url as twelvelabs_asset_preview_url,
    get_asset as twelvelabs_get_asset,
    get_analyze_task as twelvelabs_get_analyze_task,
    get_entity_collection_entity as twelvelabs_get_entity_collection_entity,
    ...
    list_entity_collection_entities as twelvelabs_list_entity_collection_entities,
    list_indexed_assets,
    list_knowledge_store_items,
    parse_json_object,
    response_id,
    retrieve_search_page as twelvelabs_retrieve_search_page,
    search_index as twelvelabs_search_index,
    search_index_with_file as twelvelabs_search_index_with_file,
...
ASSEMBLY_SEGMENT_MERGE_EPSILON_SECONDS = 0.05
...
INDEX_VIDEOS_METADATA_EXECUTOR = ThreadPoolExecutor(
    max_workers=max(1, int(os.environ.get("SPORTS_INDEX_VIDEOS_METADATA_WORKERS", "4")))
)
UPLOAD_METADATA_LOCK = Lock()
PEGASUS_SYNC_WINDOW_SECONDS = int(os.environ.get("PEGASUS_SYNC_WINDOW_SECONDS", "3300"))
STREAM_INFO_CACHE_TTL_SECONDS = int(os.environ.get("SPORTS_STREAM_INFO_CACHE_TTL_SECONDS", "60"))
STREAM_INFO_CACHE = {}
...
VIDEO_STATS_METADATA_FIELD = "sports_jockey_video_stats_v1"
VIDEO_STATS_GENERATED_AT_METADATA_FIELD = "sports_jockey_video_stats_generated_at_v1"
VIDEO_STATS_PROVIDED_METADATA_FIELD = "sports_jockey_video_stats_provided_v1"
USER_UPLOADED_VIDEO_METADATA_FIELD = "sports_jockey_user_uploaded_v1"
JOCKEY_CHAT_SCHEMA = {
    "type": "object",
    "properties": {
...
def list_game_index_videos(tag):
    ...
    with INDEX_VIDEOS_BUILD_LOCKS[tag]:
        ...
        hydrated_assets = list(
            INDEX_VIDEOS_METADATA_EXECUTOR.map(
                lambda indexed_asset: indexed_asset_with_user_metadata(index_id, indexed_asset),
                indexed_assets,
            )
        )
        videos = [normalize_index_video(index_id, indexed_asset) for indexed_asset in hydrated_assets]
        payload = {
            "index_id": index_id,
...
def register_game(payload):
    ...
    remote_indexed_ids = payload.get("marengo_video_ids", existing_game.get("marengo_video_ids", {}))
    remote_video_ids = {
        **(remote_asset_ids if isinstance(remote_asset_ids, dict) else {}),
        **(remote_indexed_ids if isinstance(remote_indexed_ids, dict) else {}),
    }
    source_videos = validate_source_videos(payload.get("source_videos", []), remote_asset_ids=remote_video_ids)
    game = {
        "tag": tag,
    ...
    video_stats = validate_video_stats(payload.get("video_stats", existing_game.get("video_stats", {})), source_videos)
    if video_stats:
        game["video_stats"] = video_stats

    video_stats_provided = validate_video_stats_provided(
        payload.get("video_stats_provided", existing_game.get("video_stats_provided", {})),
        source_videos,
    )
    if video_stats_provided:
        game["video_stats_provided"] = video_stats_provided

    user_uploaded_videos = validate_user_uploaded_videos(
        payload.get("user_uploaded_videos", existing_game.get("user_uploaded_videos", {})),
        source_videos,
    )
    if user_uploaded_videos:
        game["user_uploaded_videos"] = user_uploaded_videos
...
def upload_game_video(tag, uploaded_file, video_stats=None):
    game = get_game(tag)
    search_index_id, created_index = ensure_game_search_index(game)
...
def queue_uploaded_video_indexing(tag, video_name, asset_id, uploaded_asset, search_index_id, upload_aliases=None, video_stats=None):
...
def finish_uploaded_video_indexing(tag, video_name, asset_id, uploaded_asset, search_index_id, upload_aliases=None, video_stats=None):
    try:
        wait_for_uploaded_asset_ready(asset_id, uploaded_asset)
        game = get_game(tag)
        ...
        store_upload_index_metadata(
            search_index_id,
            indexed_asset,
            video_stats=video_stats,
            stats_provided=video_stats is not None,
        )
        ...
        update_uploaded_game_metadata(
            ...
            upload_aliases=upload_aliases,
            video_stats=video_stats,
            stats_provided_at_upload=video_stats is not None,
        )
        cache_indexed_video_thumbnail(video_name, indexed_asset, search_index_id)
        invalidate_index_videos_cache(tag)
    except Exception:
...
def store_upload_index_metadata(index_id, indexed_asset, video_stats=None, stats_provided=False):
    indexed_asset_id = response_id(indexed_asset)
    if not index_id or not indexed_asset_id:
        return None
    existing_metadata = indexed_asset_user_metadata(indexed_asset)
    merged_metadata = {
        **(existing_metadata if isinstance(existing_metadata, dict) else {}),
        USER_UPLOADED_VIDEO_METADATA_FIELD: "true",
    }
    if video_stats is not None:
        merged_metadata[VIDEO_STATS_METADATA_FIELD] = json.dumps(video_stats, ensure_ascii=False, separators=(",", ":"))
        merged_metadata[VIDEO_STATS_GENERATED_AT_METADATA_FIELD] = timestamp()
        merged_metadata[VIDEO_STATS_PROVIDED_METADATA_FIELD] = "true" if stats_provided else "false"
    try:
        update_indexed_asset_user_metadata(index_id, indexed_asset_id, merged_metadata)
    except ApiError:
        LOGGER.exception("Failed to store upload metadata on indexed asset %s", indexed_asset_id)
        return None
    return merged_metadata


def store_video_stats_index_metadata(index_id, indexed_asset, video_stats, video_name=None, stats_provided=False):
    indexed_asset_id = response_id(indexed_asset)
    if not index_id or not indexed_asset_id or video_stats is None:
        return None
    existing_metadata = indexed_asset_user_metadata(indexed_asset)
    merged_metadata = {
        **(existing_metadata if isinstance(existing_metadata, dict) else {}),
        VIDEO_STATS_METADATA_FIELD: json.dumps(video_stats, ensure_ascii=False, separators=(",", ":")),
        VIDEO_STATS_GENERATED_AT_METADATA_FIELD: timestamp(),
        VIDEO_STATS_PROVIDED_METADATA_FIELD: "true" if stats_provided else "false",
    }
    try:
        update_indexed_asset_user_metadata(index_id, indexed_asset_id, merged_metadata)
    except ApiError:
        LOGGER.exception("Failed to store uploaded stats to indexed asset %s", indexed_asset_id)
        return None
    return merged_metadata
...
def update_uploaded_game_metadata(
    ...
    upload_aliases=None,
    video_stats=None,
    stats_provided_at_upload=False,
):
    with UPLOAD_METADATA_LOCK:
        game = get_game(tag)
        return register_game(
            uploaded_game_payload(
                ...
                upload_aliases=upload_aliases,
                video_stats=video_stats,
                stats_provided_at_upload=stats_provided_at_upload,
            )
        )
...
def uploaded_game_payload(
    ...
    source_videos = upload_source_videos(game.get("source_videos", []), video_name, video_asset_ids, search_video_ids)
    video_reference_map = prune_video_reference_map(video_reference_map, source_videos)
    video_asset_ids = prune_video_keyed_map(video_asset_ids, source_videos)
    search_video_ids = prune_video_keyed_map(search_video_ids, source_videos)
    video_stats_map = deepcopy(game.get("video_stats", {})) if isinstance(game.get("video_stats"), dict) else {}
    video_stats_provided_map = deepcopy(game.get("video_stats_provided", {})) if isinstance(game.get("video_stats_provided"), dict) else {}
    user_uploaded_videos_map = deepcopy(game.get("user_uploaded_videos", {})) if isinstance(game.get("user_uploaded_videos"), dict) else {}
    video_stats_map = prune_video_keyed_map(video_stats_map, source_videos)
    video_stats_provided_map = prune_video_keyed_map(video_stats_provided_map, source_videos)
    user_uploaded_videos_map = prune_video_keyed_map(user_uploaded_videos_map, source_videos)
    if video_stats is not None:
        video_stats_map[video_name] = video_stats
    if stats_provided_at_upload and video_stats is not None:
        video_stats_provided_map[video_name] = True
    user_uploaded_videos_map[video_name] = True
    ...
    if video_stats_map:
        payload["video_stats"] = video_stats_map
    if video_stats_provided_map:
        payload["video_stats_provided"] = video_stats_provided_map
    if user_uploaded_videos_map:
        payload["user_uploaded_videos"] = user_uploaded_videos_map
...
def upload_source_videos(existing_source_videos, uploaded_video_name, video_asset_ids, search_video_ids):
    valid_source_videos = []
    for source_video in existing_source_videos if isinstance(existing_source_videos, list) else []:
        if not isinstance(source_video, str) or not source_video.strip():
            continue
        clean_name = source_video.strip()
        if (
            video_path(clean_name).exists()
            or clean_optional_string(video_asset_ids.get(clean_name))
            or clean_optional_string(search_video_ids.get(clean_name))
        ):
            valid_source_videos.append(clean_name)
            continue
        LOGGER.warning("Pruning stale source video while registering upload: %s", clean_name)
    return unique_preserving_order([*valid_source_videos, uploaded_video_name])


def prune_video_reference_map(video_reference_map, source_videos):
    source_video_set = set(source_videos)
    pruned = {}
    for reference, video_name in video_reference_map.items():
        if not isinstance(reference, str) or not isinstance(video_name, str):
            continue
        clean_name = video_name.strip()
        if clean_name in source_video_set:
            pruned[reference.strip()] = clean_name
    return pruned


def prune_video_keyed_map(values, source_videos):
    source_video_set = set(source_videos)
    pruned = {}
    for video_name, value in values.items():
        if not isinstance(video_name, str):
            continue
        clean_name = video_name.strip()
        if clean_name in source_video_set:
            pruned[clean_name] = value
    return pruned
...
def normalize_index_video(index_id, indexed_asset):
    ...
    detailed_response = parse_json_object(metadata.get(PEGASUS_DETAILED_RESPONSE_METADATA_FIELD))
    clip_counts = detailed_response.get("clip_counts") if isinstance(detailed_response.get("clip_counts"), dict) else None
    uploaded_stats = parse_json_value(metadata.get(VIDEO_STATS_METADATA_FIELD))
    stats_provided_at_upload = metadata.get(VIDEO_STATS_PROVIDED_METADATA_FIELD) == "true"
    user_uploaded = metadata.get(USER_UPLOADED_VIDEO_METADATA_FIELD) == "true"
    ...
    return {
        ...
        "has_uploaded_stats": uploaded_stats is not None and stats_provided_at_upload,
        "uploaded_stats": uploaded_stats if stats_provided_at_upload else None,
        "uploaded_stats_generated_at": clean_optional_string(metadata.get(VIDEO_STATS_GENERATED_AT_METADATA_FIELD)),
        "stats_provided_at_upload": stats_provided_at_upload,
...
def search_game_videos(tag, payload=None):
    ...
    limit = optional_payload_int(payload.get("limit"), "limit", default=12, minimum=1, maximum=12)
    page_token = clean_optional_string(payload.get("page_token"))
    if page_token:
        query_kind = clean_optional_string(payload.get("query_kind")) or "text"
        if query_kind not in {"text", "image", "entity"}:
            raise ApiError("query_kind is not supported", 400)
        return search_game_videos_with_marengo_page(game, query, limit, payload, query_kind=query_kind)
...
def search_game_videos_by_image(tag, file, payload=None):
    game = get_game(tag)
    payload = payload or {}
    limit = optional_payload_int(payload.get("limit"), "limit", default=12, minimum=1, maximum=12)
    requested_video = payload.get("video_name") or payload.get("source_video")
    ...
    query = clean_optional_string(payload.get("query")) or clean_optional_string(getattr(file, "filename", None)) or "uploaded image"
    return search_game_videos_with_marengo_image(game, query, file, limit, video_name, payload, configured_search_index_id(game))


def list_game_entities(tag):
    get_game(tag)
    collection_id = configured_entity_collection_id()
    if not collection_id:
        raise ApiError("TWELVELABS_ENTITY_COLLECTION_ID or ENTITY_COLLECTION_ID is required for entity search", 500)
    payload = twelvelabs_list_entity_collection_entities(collection_id, page=1, page_limit=50, status="ready")
    raw_entities = payload.get("data") if isinstance(payload, dict) else []
    entities = []
    if isinstance(raw_entities, list):
        for raw_entity in raw_entities:
            normalized = normalize_twelvelabs_entity(raw_entity, game_tag=tag)
            if normalized:
...
def create_game_entity(tag, name, image_file, description=None):
    get_game(tag)
    collection_id = configured_entity_collection_id()
    if not collection_id:
        raise ApiError("TWELVELABS_ENTITY_COLLECTION_ID or ENTITY_COLLECTION_ID is required for entity search", 500)
    raw_entity = twelvelabs_create_entity_collection_entity(
        collection_id,
        name,
        image_file,
        description=clean_optional_string(description),
        metadata={"source": "sports_jockey"},
    )
    entity_payload = raw_entity.get("data") if isinstance(raw_entity, dict) and isinstance(raw_entity.get("data"), dict) else raw_entity
    entity_id = response_id(entity_payload) if isinstance(entity_payload, dict) else None
    if entity_id:
        persist_entity_upload_image(entity_id, image_file)
    entity = normalize_twelvelabs_entity(entity_payload, game_tag=tag)
    return {
        "provider": "twelvelabs",
        "entity_collection_id": collection_id,
        "entity": entity or entity_payload,
    }


def search_game_videos_by_entity(tag, payload=None):
    game = get_game(tag)
    payload = payload or {}
    entity_id = required_payload_string(payload, "entity_id")
    entity_name = clean_optional_string(payload.get("entity_name")) or entity_id
    query_suffix = clean_optional_string(payload.get("query_suffix"))
    query = f"<@{entity_id}>{f' {query_suffix}' if query_suffix else ''}"
    display_query = clean_optional_string(payload.get("query")) or f"{entity_name}{f' {query_suffix}' if query_suffix else ''}"
    limit = optional_payload_int(payload.get("limit"), "limit", default=12, minimum=1, maximum=12)
    if clean_optional_string(payload.get("page_token")):
        result = search_game_videos_with_marengo_page(game, display_query, limit, payload, query_kind="entity")
        return entity_search_response(result, entity_id, entity_name)
    ...
    result = search_game_videos_with_marengo(game, query, limit, None, video_name, payload, configured_search_index_id(game))
    result = entity_search_response(result, entity_id, entity_name, display_query=display_query)
    return result


def entity_search_response(result, entity_id, entity_name, display_query=None):
    result["query"] = display_query or result.get("query") or entity_name
    result["query_kind"] = "entity"
    result["entity"] = {"id": entity_id, "name": entity_name}
    result["query_interpretation"] = f"Matched video moments containing entity: {entity_name}"
    for item in result.get("results", []):
        if isinstance(item, dict):
            item["title"] = f"{entity_name} match"
            item["description"] = item.get("description") or f"Found a visual match for {entity_name}."
    return result


def normalize_twelvelabs_entity(raw_entity, game_tag=None):
    if not isinstance(raw_entity, dict):
        return None
    entity_id = response_id(raw_entity)
    name = clean_optional_string(raw_entity.get("name"))
    if not entity_id or not name:
        return None
    asset_ids = raw_entity.get("asset_ids")
    if not isinstance(asset_ids, list):
        asset_ids = []
    return {
        "id": entity_id,
        "name": name,
        "description": clean_optional_string(raw_entity.get("description")),
        "status": clean_optional_string(raw_entity.get("status")),
        "image_url": twelvelabs_entity_image_url(raw_entity, game_tag=game_tag, entity_id=entity_id),
        "asset_ids": [asset_id for asset_id in asset_ids if isinstance(asset_id, str) and asset_id.strip()],
        "metadata": raw_entity.get("metadata") if isinstance(raw_entity.get("metadata"), dict) else {},
        "created_at": clean_optional_string(raw_entity.get("created_at")),
        "updated_at": clean_optional_string(raw_entity.get("updated_at")),
    }


def twelvelabs_entity_image_url(raw_entity, game_tag=None, entity_id=None):
    for key in ("image_url", "thumbnail_url", "picture_url", "avatar_url"):
        value = clean_optional_string(raw_entity.get(key))
        if value:
            return value
    image = raw_entity.get("image")
    if isinstance(image, dict):
        for key in ("url", "image_url", "thumbnail_url"):
            value = clean_optional_string(image.get(key))
            if value:
                return value
    images = raw_entity.get("images")
    if isinstance(images, list):
        for image_item in images:
            if isinstance(image_item, str):
                value = clean_optional_string(image_item)
                if value:
                    return value
            if isinstance(image_item, dict):
                for key in ("url", "image_url", "thumbnail_url"):
                    value = clean_optional_string(image_item.get(key))
                    if value:
                        return value
    if game_tag and entity_id and entity_has_display_image(raw_entity, entity_id):
        return entity_image_api_path(game_tag, entity_id)
    metadata_url = entity_metadata_data_url(raw_entity.get("metadata"))
    if metadata_url:
        return metadata_url
    return None


def entity_has_display_image(raw_entity, entity_id):
    if entity_metadata_image_bytes(raw_entity.get("metadata"))[0]:
        return True
    if local_entity_image_path(entity_id):
        return True
    asset_ids = raw_entity.get("asset_ids")
    return isinstance(asset_ids, list) and bool(asset_ids)


def entity_image_api_path(game_tag, entity_id):
    return f"/games/{quote(game_tag, safe='')}/entities/{quote(entity_id, safe='')}/image"


def entity_metadata_data_url(metadata):
    content, mime_type = entity_metadata_image_bytes(metadata)
    if not content:
        return None
...
def entity_metadata_image_bytes(metadata):
    if not isinstance(metadata, dict):
        return None, None
    for key in ("face_snap_base64", "image_base64", "thumbnail_base64", "snap_base64"):
        ...
        if value.startswith("data:"):
            ...
            try:
                return base64.b64decode(payload), mime_type or "image/png"
            except (ValueError, binascii.Error):
                continue
        try:
            return base64.b64decode(value), entity_metadata_image_mime(metadata, value)
        except (ValueError, binascii.Error):
            continue
    return None, None


def entity_metadata_image_mime(metadata, base64_value):
    for key in ("face_snap_mime", "image_mime", "mime_type"):
        mime_type = clean_optional_string(metadata.get(key))
        if mime_type:
            return mime_type
    if base64_value.startswith("/9j/"):
        return "image/jpeg"
    if base64_value.startswith("R0lGOD"):
        return "image/gif"
    if base64_value.startswith("UklGR"):
        return "image/webp"
    return "image/png"


def local_entity_image_path(entity_id):
    entity_id = clean_optional_string(entity_id)
    if not entity_id:
        return None
    for suffix in (".png", ".jpg", ".jpeg", ".webp", ".gif"):
        path = ENTITY_IMAGES_DIR / f"{entity_id}{suffix}"
        if path.exists():
            return path
    return None


def persist_entity_upload_image(entity_id, image_file):
    entity_id = clean_optional_string(entity_id)
    if not entity_id or image_file is None:
        return None
    filename = clean_optional_string(getattr(image_file, "filename", None)) or "entity.png"
...
def fetch_twelvelabs_entity(entity_id):
    collection_id = configured_entity_collection_id()
    if not collection_id:
        raise ApiError("TWELVELABS_ENTITY_COLLECTION_ID or ENTITY_COLLECTION_ID is required for entity search", 500)
    return twelvelabs_get_entity_collection_entity(collection_id, entity_id)


def entity_image_bytes(tag, entity_id):
    get_game(tag)
    entity_id = clean_optional_string(entity_id)
    if not entity_id:
        raise ApiError("entity_id is required", 400)

    local_path = local_entity_image_path(entity_id)
    if local_path:
        return local_path.read_bytes(), entity_image_mime_for_path(local_path)

    raw_entity = fetch_twelvelabs_entity(entity_id)
    content, mime_type = entity_metadata_image_bytes(raw_entity.get("metadata"))
    if content:
    ...
    if isinstance(asset_ids, list):
        for asset_id in asset_ids:
            ...
            try:
                ...
                continue
            preview_url = twelvelabs_asset_preview_url(asset)
            if preview_url:
                response = requests.get(preview_url, timeout=REQUEST_TIMEOUT_SECONDS)
                response.raise_for_status()
                mime_type = clean_optional_string(response.headers.get("Content-Type")) or "image/jpeg"
                return response.content, mime_type

    raise ApiError("entity image not found", 404)


def entity_image_mime_for_path(path):
...
def search_game_videos_with_marengo_page(game, query, limit, payload, query_kind="text"):
    search_options = validate_marengo_search_options(payload.get("search_options"))
    group_by = validate_marengo_group_by(payload.get("group_by"))
    result = twelvelabs_retrieve_search_page(payload.get("page_token"), include_user_metadata=True)
    return normalize_marengo_search(game, query, result, limit, search_options, group_by, query_kind=query_kind)


def search_game_videos_with_marengo_image(game, query, file, limit, video_name, payload, marengo_index_id):
    search_options = validate_marengo_search_options(payload.get("search_options"))
    group_by = validate_marengo_group_by(payload.get("group_by"))
    fields = [
        ("index_id", marengo_index_id),
    ...
    result = twelvelabs_search_index_with_file(fields, "query_media_file", file)
    return normalize_marengo_search(game, query, result, limit, search_options, group_by, query_kind="image")
...
def create_selected_clip_analysis(tag, payload=None):
    ...
    payload = payload or {}
    force_generate = bool(payload.get("force_generate") or payload.get("regenerate"))
    analysis_task_id = clean_optional_string(payload.get("analysis_task_id")) or clean_optional_string(payload.get("task_id"))
    ...
    analysis_payload = selected_clip_analysis_request_payload(asset_id, analyze_start, analyze_end, prompt)
    if analysis_task_id:
        result = twelvelabs_get_analyze_task(analysis_task_id)
        task_status = clean_optional_string(result.get("status"))
        if task_status != "ready":
            return selected_clip_analysis_task_response(
                game=game,
                video_name=video_name,
                start_time=start_time,
                end_time=end_time,
    ...
    elif selected_clip_analysis_requires_async(asset_id):
        result = twelvelabs_create_analyze_task(analysis_payload)
        return selected_clip_analysis_task_response(
            game=game,
            video_name=video_name,
            start_time=start_time,
        ...
        result = twelvelabs_analyze_video(analysis_payload)
...
def selected_clip_analysis_request_payload(asset_id, analyze_start, analyze_end, prompt):
    return {
...
def selected_clip_analysis_requires_async(asset_id):
    try:
        asset = twelvelabs_get_asset(asset_id)
    except ApiError:
        return False
    duration = twelvelabs_asset_duration_seconds(asset)
    return bool(duration and duration > 3600)


def selected_clip_analysis_task_response(
    *,
    game,
    video_name,
    start_time,
    ...
    task_status = clean_optional_string(task.get("status")) or "processing"
    task_id = response_id(task)
    if task_status == "failed":
        raise ApiError(
            {
                "message": "Pegasus selected clip analysis failed",
                "task_id": task_id,
    ...
    return {
        ...
        "model": TWELVELABS_PEGASUS_MODEL,
        "source": "pegasus_clip_analysis",
        "status": task_status if task_status == "ready" else "processing",
        "task_id": task_id,
        "game_tag": game["tag"],
        "video_name": video_name,
        ...
        "_jockey_metadata": {
            "source": "pegasus_clip_analysis_async_task",
            "from_user_metadata": False,
...
def selected_clip_analysis_prompt(
    ...
    parts = [
        ...
        "Focus on the selected clip. Use nearby padded context only to understand the moment.",
        "Generate producer-ready structured metadata: description, emotional_tone, key_action, participants, moment_types, tags, score_context, producer_summary, story_arc, editorial_use, recommended_formats, evidence lists, and confidence.",
...
def parse_selected_clip_analysis_json(result):
    if isinstance(result, dict):
        ...
        if isinstance(nested_data, dict):
            try:
                return parse_selected_clip_analysis_json(nested_data)
            except ApiError:
                pass
...
def create_jockey_chat_response(tag, payload=None):
    ...
    limit = optional_payload_int(payload.get("limit"), "limit", default=None, minimum=0)
    if include_reel and limit is None:
        limit = jockey_requested_clip_count(message)
    if not include_reel:
        limit = 0
...
def jockey_chat_prompt(game, message, limit, video_name=None, include_reel=False, conversation_history=None):
    ...
    if include_reel:
        clip_instruction = (
            f"Return exactly {limit} ranked clips when that many grounded clips exist; otherwise return every grounded clip you can support, up to {limit}."
            if limit is not None
            else "Return all grounded ranked clips that are useful for a typed timestamp manifest that can be rendered as a simple Jockey reel or clip showcase."
        )
        parts.extend(
            [
                clip_instruction,
                "The clips should form a typed timestamp manifest that can be rendered as a simple Jockey reel or clip showcase.",
                ...
                "Keep each jockey_rationale to one concise sentence focused on why this exact moment matters.",
    ...
    parts.append("If no grounded matches exist, return an empty clips array with a brief narrative_summary explaining what was searched and what was missing.")
    return "\n".join(parts)
...
def jockey_requested_clip_count(message):
    if not isinstance(message, str) or not message.strip():
        return None
    clip_terms = r"(?:reels?|clips?|moments?|showcases?|plays?|highlights?)"
    qualifiers = r"(?:(?:best|top|ranked|strongest|grounded|reel|highlight|playable)\s+){0,4}"
    digit_patterns = (
        rf"\b(\d{{1,4}})\s+{qualifiers}{clip_terms}\b",
        rf"\b(?:best|top|first|give\s+me|show\s+me|find|return)\s+(\d{{1,4}})\s+{qualifiers}{clip_terms}\b",
    )
    ...
    word_group = "|".join(re.escape(word) for word in JOCKEY_CLIP_COUNT_WORDS)
    word_patterns = (
        rf"\b({word_group})\s+{qualifiers}{clip_terms}\b",
        rf"\b(?:best|top|first|give\s+me|show\s+me|find|return)\s+({word_group})\s+{qualifiers}{clip_terms}\b",
    )
    for pattern in word_patterns:
        match = re.search(pattern, message, flags=re.IGNORECASE)
        if match:
            return JOCKEY_CLIP_COUNT_WORDS[match.group(1).lower()]
    return None
...
def normalize_jockey_chat_manifest(game, message, result, manifest, limit):
    ...
    limited_raw_clips = raw_clips if limit is None else raw_clips[:limit]
    clips = []
    for index, raw_clip in enumerate(limited_raw_clips):
    ...
    return {
        ...
        "requested_clip_count": limit if isinstance(limit, int) and limit > 0 else None,
        "clips": clips,
    }
...
def normalize_marengo_search(game, query, result, limit, search_options, group_by, query_kind="text"):
    ...
    page_info = normalize_marengo_page_info(result.get("page_info"))
    ...
    return {
        ...
        "query_interpretation": (
            f"Matched visual/audio evidence similar to the uploaded image: {query}"
            if query_kind == "image"
            else f"Matched visual/audio evidence for: {query}"
        ),
        "total_results": total_results if isinstance(total_results, int) and total_results >= len(results) else len(results),
        "search_options": search_options,
        "group_by": group_by,
        "page_info": page_info,
        "search_pool": {
...
def normalize_marengo_page_info(raw_page_info):
    ...
    for key in (
        ...
        value = raw_page_info.get(key)
        if value is not None:
            page_info[key] = value
    return page_info or None
...
def parse_assembly_segments(value):
    ...
    return merge_adjacent_assembly_segments(ranges)


def merge_adjacent_assembly_segments(ranges):
    merged = []
    for start_seconds, end_seconds in ranges:
        if merged:
            previous_start, previous_end = merged[-1]
            chronological = start_seconds >= previous_start - ASSEMBLY_SEGMENT_MERGE_EPSILON_SECONDS
            touches_previous = start_seconds <= previous_end + ASSEMBLY_SEGMENT_MERGE_EPSILON_SECONDS
            if chronological and touches_previous:
...
def resolve_workspace_video_target(
    ...
    *,
    ...
    for candidate in references:
        indexed_asset = indexed_asset_for_reference(
            index_id,
            candidate,
...
def validate_user_uploaded_videos(user_uploaded_videos, source_videos):
    if user_uploaded_videos is None:
        return {}
    if not isinstance(user_uploaded_videos, dict):
        raise ApiError("user_uploaded_videos must be an object", 400)

    source_video_set = set(source_videos)
    validated = {}
    for video_name, uploaded in user_uploaded_videos.items():
        if not isinstance(video_name, str) or not video_name.strip():
            raise ApiError("user_uploaded_videos keys must be source video names", 400)
        clean_video_name = video_name.strip()
        if clean_video_name not in source_video_set:
            raise ApiError(f"user_uploaded_videos key is not a registered source video: {clean_video_name}", 400)
        if not isinstance(uploaded, bool):
            raise ApiError("user_uploaded_videos values must be booleans", 400)
        if uploaded:
            validated[clean_video_name] = True
    return validated


def validate_video_stats_provided(video_stats_provided, source_videos):
    if video_stats_provided is None:
        return {}
    if not isinstance(video_stats_provided, dict):
        raise ApiError("video_stats_provided must be an object", 400)

    source_video_set = set(source_videos)
    validated = {}
    for video_name, provided in video_stats_provided.items():
        if not isinstance(video_name, str) or not video_name.strip():
            raise ApiError("video_stats_provided keys must be source video names", 400)
        clean_video_name = video_name.strip()
        if clean_video_name not in source_video_set:
            raise ApiError(f"video_stats_provided key is not a registered source video: {clean_video_name}", 400)
        if not isinstance(provided, bool):
            raise ApiError("video_stats_provided values must be booleans", 400)
        if provided:
            validated[clean_video_name] = True
    return validated


def validate_video_stats(video_stats, source_videos):
    if video_stats is None:
        return {}
    if not isinstance(video_stats, dict):
        raise ApiError("video_stats must be an object", 400)

    source_video_set = set(source_videos)
    validated = {}
    for video_name, stats in video_stats.items():
        if not isinstance(video_name, str) or not video_name.strip():
            raise ApiError("video_stats keys must be source video names", 400)
        clean_video_name = video_name.strip()
        if clean_video_name not in source_video_set:
            raise ApiError(f"video_stats key is not a registered source video: {clean_video_name}", 400)
        if not isinstance(stats, (dict, list)):
            raise ApiError("video_stats values must be a JSON object or array", 400)
        validated[clean_video_name] = deepcopy(stats)
    return validated
