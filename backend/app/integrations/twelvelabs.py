...
DIRECT_UPLOAD_LIMIT_BYTES = 200 * 1000 * 1000
...
def entity_collection_endpoint(entity_collection_id):
    return f"{ENTITY_COLLECTIONS_ENDPOINT}/{entity_collection_id}"


def entity_collection_entities_endpoint(entity_collection_id):
    return f"{entity_collection_endpoint(entity_collection_id)}/entities"
...
def request_json(method, endpoint, payload=None):
    attempts = (
        ...
        if endpoint == ANALYZE_ENDPOINT or endpoint.startswith(ANALYZE_TASKS_ENDPOINT)
        else 1
    )
...
def request_form_with_files(method, endpoint, fields, file_fields):
    form_fields = []
    upload_fields = []
    open_files = []
    try:
        ...
        for key, file_storage in file_fields:
            if file_storage is None:
                continue
            filename = clean_optional_string(getattr(file_storage, "filename", None)) or "query-image"
            mimetype = (
                clean_optional_string(getattr(file_storage, "mimetype", None))
            ...
            try:
                ...
                pass
            upload_fields.append((key, (filename, stream, mimetype)))
...
def create_analyze_task(payload):
    return request_json("post", ANALYZE_TASKS_ENDPOINT, payload)


def get_analyze_task(task_id):
    task_id = clean_optional_string(task_id)
    if not task_id:
        raise ApiError("TwelveLabs async analysis task id is required", 400)
    return request_json("get", f"{ANALYZE_TASKS_ENDPOINT}/{task_id}")


def analyze_video_async(
    payload,
    max_attempts=ANALYZE_TASK_STATUS_ATTEMPTS,
    interval_seconds=ANALYZE_TASK_STATUS_INTERVAL_SECONDS,
):
    created_task = create_analyze_task(payload)
    task_id = clean_optional_string(created_task.get("task_id")) or response_id(created_task)
    if not task_id:
        raise ApiError(
            {
                "message": "TwelveLabs async analysis task response did not include task_id",
                "response": created_task,
            },
            502,
        )
    ...
    for attempt in range(1, max_attempts + 1):
        status = clean_optional_string(task.get("status"))
        if status == "ready":
            return task
        if status == "failed":
            raise ApiError(
                {
                    "message": "TwelveLabs async analysis task failed",
                    "task_id": task_id,
        ...
        task = get_analyze_task(task_id)

    raise ApiError(
        {
            "message": "TwelveLabs async analysis task did not complete before timeout",
            "task_id": task_id,
...
def search_index_with_file(fields, file_field_name, file_storage):
    if TwelveLabs is None:
        raise ApiError("twelvelabs SDK is required for image search", 500)
    if file_field_name != "query_media_file":
        return request_form_with_files("post", SEARCH_ENDPOINT, fields, [(file_field_name, file_storage)])

    field_values = sdk_search_field_values(fields)
    filename = clean_optional_string(getattr(file_storage, "filename", None)) or "query-image"
    mimetype = (
        clean_optional_string(getattr(file_storage, "mimetype", None))
        or mimetypes.guess_type(filename)[0]
    ...
    if not api_key:
        raise ApiError("TWELVELABS_API_KEY is required", 500)
    client = TwelveLabs(api_key=api_key, base_url=TWELVELABS_BASE_URL)
    try:
        result = client.search.create(
            index_id=field_values["index_id"],
            search_options=field_values["search_options"],
            query_media_type="image",
            query_media_file=(filename, stream, mimetype),
            group_by=field_values.get("group_by"),
            operator=field_values.get("operator"),
            page_limit=field_values.get("page_limit"),
            filter=field_values.get("filter"),
            include_user_metadata=field_values.get("include_user_metadata"),
        )
...
def retrieve_search_page(page_token, include_user_metadata=True):
    clean_page_token = clean_optional_string(page_token)
    if not clean_page_token:
        raise ApiError("page_token is required", 400)
    endpoint = query_endpoint(
        f"{SEARCH_ENDPOINT}/{quote(clean_page_token, safe='')}",
        {"include_user_metadata": str(bool(include_user_metadata)).lower()},
    )
    return request_json("get", endpoint)


def sdk_search_field_values(fields):
    values = {}
    search_options = []
    for key, value in fields:
        if key == "search_options":
            if isinstance(value, (list, tuple)):
                search_options.extend(str(item) for item in value if item)
            elif value:
                search_options.extend(part.strip() for part in str(value).split(",") if part.strip())
            continue
        values[key] = value
    index_id = clean_optional_string(values.get("index_id"))
    if not index_id:
        raise ApiError("index_id is required for TwelveLabs image search", 500)
    values["index_id"] = index_id
    values["search_options"] = search_options or ["visual"]
    if values.get("page_limit") is not None:
        values["page_limit"] = int(values["page_limit"])
    if values.get("include_user_metadata") is not None:
        values["include_user_metadata"] = str(values["include_user_metadata"]).lower() == "true"
    return values
...
# Entity collections
def list_entity_collection_entities(entity_collection_id, page=1, page_limit=50, status="ready"):
    return request_json(
        "get",
        query_endpoint(
            entity_collection_entities_endpoint(entity_collection_id),
            {
                "page": page,
...
def get_entity_collection_entity(entity_collection_id, entity_id):
    return request_json("get", f"{entity_collection_entities_endpoint(entity_collection_id)}/{entity_id}")


def asset_preview_url(asset):
    if not isinstance(asset, dict):
        return None
    thumbnail = asset.get("thumbnail")
    if isinstance(thumbnail, dict):
        status = clean_optional_string(thumbnail.get("status"))
        if status in (None, "ready"):
            url = clean_optional_string(thumbnail.get("representative_url"))
            if url:
                return url
    return indexed_asset_thumbnail_url(asset)


def create_entity_collection_entity(entity_collection_id, name, image_file, description=None, metadata=None):
    fields = [("name", name)]
    if description:
        fields.append(("description", description))
    if metadata:
        fields.append(("metadata", json.dumps(metadata)))
    endpoint = entity_collection_entities_endpoint(entity_collection_id)
    try:
        return request_form_with_files("post", endpoint, fields, [("image_file", image_file)])
    except ApiError as exc:
        if exc.status_code not in (400, 415, 422):
            raise
    return request_form_with_files("post", endpoint, fields, [("image", image_file)])
...
def indexed_asset_for_reference(index_id, reference, hydrate_metadata=True):
    reference = clean_optional_string(reference)
    if not reference:
        return None

    if looks_like_twelvelabs_object_id(reference):
        try:
...
def looks_like_twelvelabs_object_id(value):
...
def response_id(value):
    if not isinstance(value, dict):
        return None
    return (
        clean_optional_string(value.get("_id"))
        or clean_optional_string(value.get("id"))
        or clean_optional_string(value.get("task_id"))
    )
...
def parse_response(response):
    if response.status_code == 204 or not response.content:
        if response.status_code >= 400:
            raise ApiError(upstream_error_message(response), response.status_code)
        return {}
    ...
    try:
        ...
        raise ApiError(upstream_error_message(response), response.status_code if response.status_code >= 400 else 502) from exc

    if response.status_code >= 400:
        LOGGER.warning(
            "TwelveLabs API error %s for %s %s: %s",
            response.status_code,
            response.request.method if response.request else "request",
            response.url,
            data,
        )
...
def upstream_error_message(response):
    ...
    url = response.url or "TwelveLabs API"
    LOGGER.warning(
        "TwelveLabs non-JSON error %s for %s %s (%s): %s",
        response.status_code,
        method,
        url,
    ...
    if "<html" in text.lower():
        return {
            "message": f"TwelveLabs rejected the upload with {response.status_code} {response.reason or 'Bad Request'}",
            "upstream_status": response.status_code,
            "upstream_content_type": content_type,
        }
    return {
        "message": text or f"TwelveLabs returned {response.status_code} {response.reason or 'error'}",
        "upstream_status": response.status_code,
        "upstream_content_type": content_type,
    }
