...
DEFAULT_MAX_UPLOAD_VIDEO_BYTES = 400 * 1000 * 1000
UPLOAD_REQUEST_OVERHEAD_BYTES = 5 * 1000 * 1000
...
def max_upload_video_bytes():
    return int(os.environ.get("SPORTS_MAX_UPLOAD_VIDEO_BYTES", str(DEFAULT_MAX_UPLOAD_VIDEO_BYTES)))


def max_upload_request_bytes():
    return max_upload_video_bytes() + UPLOAD_REQUEST_OVERHEAD_BYTES
...
def entity_collection_id():
    return (
        os.environ.get("TWELVELABS_ENTITY_COLLECTION_ID", "").strip()
        or os.environ.get("ENTITY_COLLECTION_ID", "").strip()
    )
...
DEFAULT_CORS_ALLOWED_ORIGINS = (
    "https://tl-sports.vercel.app,"
    "https://sports-semantic-jockey.vercel.app,"
    "http://localhost:5173,"
    "http://127.0.0.1:5173"
)
...
def cors_allowed_origins():
    ...
    configured_app_url = os.environ.get("APP_URL", "").strip().rstrip("/")
    if configured_app_url and "*" not in origins and configured_app_url not in origins:
        origins.append(configured_app_url)
    return origins
