...
def optional_uploaded_stats():
    raw = request.form.get("stats")
    if raw is None or not raw.strip():
        return None
    try:
        parsed = json.loads(raw)
    except (ValueError, TypeError):
        raise ApiError("stats must be valid JSON", 400)
    if not isinstance(parsed, (dict, list)):
        raise ApiError("stats must be a JSON object or array", 400)
    return parsed
...
def uploaded_image_file():
    ...
    if not mimetype.startswith("image/"):
        raise ApiError("file must be an image", 400)
    return file
